...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'REGISTRATION FAILED. SYSTEM REJECTION.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bg-primary px-4 py-20 relative overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none overflow-hidden">
        <h1 className="font-display text-[25vw] font-black leading-none text-bg-elevated/5 italic tracking-tighter uppercase whitespace-nowrap">
          NEW&nbsp;ID.
        </h1>
      </div>
      ...
      >
        <div className="space-y-12">
          <div className="text-center space-y-4">
             ...
             <p className="font-mono text-[10px] font-bold uppercase tracking-[0.3em] text-text-dim">REQUESTING NEW SYSTEM ACCESS</p>
          </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="space-y-3">
                    <Label htmlFor="name" className="font-mono text-[10px] font-bold uppercase tracking-widest text-text-dim ml-1">ENTITY NAME</Label>
                    <Input
                      id="name"
                  ...
                  <div className="space-y-3">
                    <Label htmlFor="password" className="font-mono text-[10px] font-bold uppercase tracking-widest text-text-dim ml-1">SECRET KEY</Label>
                    <Input
                      id="password"
                      type="password"
                      placeholder="••••••••"
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      CREATE IDENTITY <UserPlus className="ml-2 h-5 w-5" />
                    </>
                  )}
                </Button>
              </form>
          ...
          <p className="text-center font-mono text-[10px] font-bold uppercase tracking-widest text-text-dim">
            EXISTING ENTITY?{' '}
            <Link href="/login" className="text-text-primary hover:text-lime-accent transition-colors border-b border-text-dim hover:border-lime-accent">
              INITIALIZE SESSION
            </Link>
          </p>
        </div>
